...
It is not obvious from the figure[fn:integral-curves] above but the lift is performed such that it projects back down to \(\tau\) via \(T\pi\). Since \(T\pi \colon (\dot{x}, \dot{y}) \mapsto \dot{x}\), the form the vector field must take on \(Y\) is

\begin{align}
\Gamma \colon \partial_x & \mapsto \partial_x + C \partial_y, & C \in \mathbb{R}.
\end{align}
...
If we have two vector fields \(\overline{u}, \overline{v} \colon \mathbb{R}^n \to \mathbb{R}^n\) there is a simple method to differentiate \(\overline{v}\) in the direction of \(\overline{u}\) at \(x\in\mathbb{R}^n\):
...
* Connections as infinitesimal holonomy

This is the Ambrose-Singer theorem, first stated in [cite:@ambrose-singer-1953]; exposition in [cite:@kobayashi-nomizu-1996] Chapter 2 §7.8 for principal G-bundles, and an extension on fiber bundles in [cite:@michor-1991] §12.4.

* Connections as global sections of jet bundle

The connections of a fiber bundle \(\pi\colon Y \to X\) are in bijection with the global sections of the first jet bundle \(J^1Y \to Y\): the components have the same transformation laws and hence one can be used to define the other. On the other hand, \(J^1Y \to Y\) is an affine bundle modelled over the vector bundle \(T^*X \underset{Y}{\otimes} VY \to Y\).[fn:jet-affine-bundle] A global section \(\Gamma \colon Y \to J^1Y\) provides an isomorphism of these two bundles:

\begin{equation}
\begin{aligned}
D_\Gamma \colon J^1Y &\xrightarrow[Y]{\cong} T^*X \underset{Y}{\otimes} VY, \\
y^i_\lambda & \mapsto (y^i_\lambda - \Gamma^i_\lambda)dx^\lambda\otimes \partial_i.
\end{aligned}
\end{equation}

The isomorphism above merely chooses an origin \(\Gamma^i_\lambda(y)\) for the affine space \(J^1_{y}Y\) for each \(y\in Y\). What is interesting is that \(D_\Gamma\) is the /covariant differential/! In fact the covariant derivative is recovered immediately as \(\nabla^\Gamma s = D_\Gamma \circ j^1s\) for a section \(s \colon X \to Y\). Let us explain the prolongation \(j^1s\) here: the section \(s\) makes the \(y^i\) variables depend on \(x^\lambda\), i.e. \(y^i = s^i(x)\); all \(j^1s\) does is make \(y^i_\lambda\) also depend on \(x\), i.e. \(y^i_\lambda = \partial_\lambda s^i(x)\).

[fn:jet-affine-bundle] More generally, we have that the affine bundle \(\pi_{k,k-1}\colon J^kY \to J^{k-1}Y\) is modelled over the vector bundle \(S^kT^*X \underset{J^{k-1}Y}{\otimes} VY \to J^{k-1}Y\), see Proposition 2.1.13 of [cite:@saunders-thesis].
