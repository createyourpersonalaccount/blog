...
Imagine that a complex phase is travelling on the straight line segment connecting the origin $O$ and a point $P$. It starts at $P$ with the value $1$ and travels along the straight line in unit speed, while rotating with angular frequency $\omega$. At time $t$, we find the phase to be $e^{i\omega t}$. Moving at unit speed, it reaches the origin $O$ exactly in $r = \|OP\|$ time; thus it reaches the origin with the value $e^{i\omega r}$. (The value of $\omega$ decides that 1 space unit corresponds to $\omega$ frequency units.)

## The interpretation of the dot product

Now let $x\in\mathbb{R}^n$ be the position of the point $P$ under consideration. In the Fourier transform, the phase $e^{-2\pi i x\cdot\xi}$ is involved. Here the negative sign is in fact on $x$; the vector $-x$ is the vector starting from $P$ pointing back at the origin. Let us set $k = \omega \xi$. The vector $k$ is the wave vector and contains the direction of travel of the plane wave. Plane waves arise as special solutions of the source-free wave equation (see § 5.5 of [Advanced Classical Electromagnetism by R. M. Wald](https://press.princeton.edu/books/hardcover/9780691220390/advanced-classical-electromagnetism).) Their form is $Ce^{i(x\cdot k - \omega t)}$ for given $C\in\mathbb{C}^d$ (here we take $d=1$) and $\omega\in\mathbb{R}$. They are travelling in the direction of $k$ with velocity $\omega/\|k\|$. (For electromagnetic waves, $\omega = c k$.)
If we imagine that the point $P$ emits a plane wave at time $t = 0$ starting with phase $1$ with wave vector $k$ and angular frequency $\omega$, its value when it meets the origin is going to be $e^{-i\omega x\cdot\xi}$. Typical values for $\omega$ are $1$ and $2\pi$.
